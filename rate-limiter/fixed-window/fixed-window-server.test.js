...
const SERVER_URL = "http://localhost:3000/fixed-window";
const CAPACITY = 5;
...
(async () => {
  ...
  console.log(` Fixed-Window Server Test`);
  console.log(
    ` Capacity: ${CAPACITY} requests | Window: ${WINDOW_MS / 1000}s`,
  );
  ...
  console.log(
    `Phase 2 — Waiting ${WINDOW_MS / 1000}s for the window to reset...\n`,
  );
  ...
  for (const user of USERS) {
    ...
    console.log(`  User: ${user} (after window reset)`);
    ...
    if (retryLog.accepted.length !== CAPACITY) {
      allGood = false;
      console.log(`    ✗ Expected ${CAPACITY} accepted after reset`);
    } else {
      console.log(`    ✓ Window reset — full capacity available again`);
    }
    console.log();
  }
